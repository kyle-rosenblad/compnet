...
As an alternative to the binomial likelihood, 'compnet' models can also be built with a [beta-binomial](https://cran.r-project.org/web/packages/brms/vignettes/brms_customfamilies.html) likelihood by changing the 'family' argument in buildcompnet() to "beta-binomial". This option can sometimes help resolve the kinds of problems that 'DHARMa' checks reveal, like overdispersion. Before turning to fancy solutions like these, it's often worthwhile to rethink our choices of predictor variables. However, that doesn't always work, so it's nice to have fancier model structures available in case we need them.
